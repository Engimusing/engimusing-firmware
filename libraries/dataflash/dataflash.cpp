...
#define WRITEPROTECTPin 3//ss
#define RESETPin 2//ss
...
{
  return mySpi.transfer(-1, output);; 
}
 ...
 *			 
 ******************************************************************************/
void Dataflash::Buffer_To_PageNE (unsigned char BufferNo, unsigned int PageAdr)
{
  DF_CS_inactive();						//make sure to toggle CS signal in order
  DF_CS_active();						//to reset Dataflash command decoder
  if (1 == BufferNo)						//program flash page from buffer 1
  {
    DF_SPI_RW(Buf1ToFlash);					//buffer 1 to flash no erase op-code
    DF_SPI_RW((unsigned char)(PageAdr >> (16 - PageBits)));	//upper part of page address
    DF_SPI_RW((unsigned char)(PageAdr << (PageBits - 8)));	//lower part of page address
    DF_SPI_RW(0x00);										//don't cares
  }
//#ifdef USE_BUFFER2
    ...
    //if (2 == BufferNo)						//program flash page from buffer 2
  {
    DF_SPI_RW(Buf2ToFlash);					//buffer 2 to flash no erase op-code
    DF_SPI_RW((unsigned char)(PageAdr >> (16 - PageBits)));	//upper part of page address
    DF_SPI_RW((unsigned char)(PageAdr << (PageBits - 8)));	//lower part of page address
    DF_SPI_RW(0x00);						//don't cares
  }
